...
void set_parameters(struct Parameters *p_parameters)
{
  ...
  p_parameters->particle_types[1].mass = 30.07/2;            // Ethane mass (amu)
  ...
  p_parameters->num_part = 1600;                            //number of particles
  ...
  p_parameters->dt = 0.1;                                  //integration time step
  
  // Here i make sure that the box size is big enough. No reason for it, it just works
  p_parameters->L = (struct Vec3D){59, 59, 59};//box size
  ...
  p_parameters->r_cut = 10;             //cut-off distance used for neigbor list
  p_parameters->r_shell = 1.6;          //shell thickness for neighbor list
