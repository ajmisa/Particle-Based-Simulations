...
double calculate_forces_nb(struct Parameters *p_parameters, struct Nbrlist *p_nbrlist, struct Vectors *p_vectors)
{
    ...
    sigmasq = p_parameters->sigma_mixed * p_parameters->sigma_mixed;
    double epsilon_mixed = p_parameters->epsilon_mixed;
    ...
    for (size_t k = 0; k < num_nbrs; k++)
    {
        ...
        if (rij.sq < r_cutsq)
        {
            ...
            Epot += 4.0 * epsilon_mixed * (sr12 - sr6 - Epot_cutoff);

            // Compute the force and apply it to both particles
            fr = 24.0 * epsilon_mixed * (2.0 * sr12 - sr6) / rij.sq;  // Force divided by distance
            df.x = fr * rij.x;
            df.y = fr * rij.y;
            df.z = fr * rij.z;
